    ...
    with self.robot:
      ...
      try:
        ...
        for c in contacts:
          ...
          #print("pos= ",pos);
          #print("dir=", dir);
          #print("cross= " ,np.cross(pos,dir));
          w = np.append(pos, np.cross(pos,dir));
          wt = np.reshape(w,(6,1));
          #print("w transpose = ",wt);
          #G =[w1,w2,w3, w4, wn
          #print(G);
...
#            finestep              = None )
  ...
  def sample_random_grasp(self, grasp_in):
    ...
    grasp[self.graspindices['igraspdir']] = dir_orig + np.random.normal(0,RAND_ANGLE_SIGMA,(1,3))
